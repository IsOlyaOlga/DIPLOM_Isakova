...
Процедура ВКМ_ЗаполнитьГрафикРаботы(ДатаНачала, ДатаОкончания, ВыходныеДни) Экспорт 
	
	Набор = РегистрыСведений.ВКМ_ГрафикиРаботыЗаполненные.СоздатьНаборЗаписей();
	...
	Для к = 0 По Набор.Количество()-1 Цикл
		...
		Иначе
			Пока Дат < Мин(Запись.Дата, ДатаОкончания) Цикл
				...
				Если Найти(ВыходныеДни, Строка(ДеньНедели(Дат))) Тогда
					НоваяЗапись.ЧасовВДень = 0;
				Иначе	          
					НоваяЗапись.ЧасовВДень = 8;
				КонецЕсли; 
				Дат = Дат + ЧислоСекундВСутках;
			КонецЦикла; 
